...
export async function GET(_req: NextRequest, { params }: Ctx) {
  const rows = (await sql`SELECT * FROM applications WHERE id = ${params.id}`) as Application[];
...
export async function PUT(req: NextRequest, { params }: Ctx) {
  ...
  const [updated] = (await sql`SELECT * FROM applications WHERE id = ${params.id}`) as Application[];
