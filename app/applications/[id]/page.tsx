...
import { ArrowLeft, Clock, Pencil, Trash2, ExternalLink, FileText, FileSignature, X } from "lucide-react";
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          <div className="flex gap-2 shrink-0 self-start">
            <button onClick={() => setEditing(!editing)}
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 font-medium transition-colors">
              {editing ? <X className="w-3.5 h-3.5" /> : <Pencil className="w-3.5 h-3.5" />}
              {editing ? "Cancel" : "Edit"}
            </button>
            <button onClick={handleDelete}
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-sm border border-red-200 dark:border-red-900 rounded-lg text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 font-medium transition-colors">
